...
class QLedLabel(QLabel):
    ...
    grey_ss = (
        "color: white;background-color: qlineargradient(spread:pad, x1:0.145, "
        "y1:0.16, x2:1, y2:1, stop:0 rgba(200, 200, 200, 120), stop:1 rgba(220, 220, 220, 255));"
    )
    green_ss = (
        "color: white;background-color: qlineargradient(spread:pad, x1:0.145, y1:0.16, "
    ...
    orange_ss = (
        "color: white;background-color: qlineargradient(spread:pad, x1:0.232, y1:0.272, "
        "x2:0.98, y2:0.959773, stop:0 rgba(255, 113, 4, 255), stop:1 rgba(91, 41, 7, 255));"
    )
    blue_ss = (
        "color: white;background-color: qlineargradient(spread:pad, x1:0.04, y1:0.0565909, "
        "x2:0.799, y2:0.795, stop:0 rgba(203, 220, 255, 255), stop:0.41206 rgba(0, 115, 255, 255), stop:1 rgba("
        "0, 49, 109, 255));"
    )
    yellow_ss = (
        "color: white;background-color: qlineargradient(spread:pad, x1:0.04, y1:0.0565909, "
        "x2:0.799, y2:0.795, stop:0 rgba(255, 255, 0, 255), stop:1 rgba(155, 155, 0, 255));"
    )
    ...
    def set_led_color(self, state_color):
        ...
        if state_color in self.led_colors:
            ...
            state_ss = (
                "QLabel{ " + self.led_colors[state_color] + "border-radius: " + str(self.minimumHeight() / 2) + "px;}"
            )
